...
int main(int argc, char **argv)
{
    ...
    //SLAM.SaveTrajectoryTUM("/home/jy/Desktop/ORB_SLAM2_AddSemantic/TUM_fr3_walking_xyz_results/CameraTrajectory2.txt");
...
void LoadBoundingBox(const string& strPathToDetectionResult, vector<std::pair<vector<double>, int>>& detect_result){
    ...
    while (getline(infile, line)){
        ...
        if (class_label == "indicative" ||   //低动态物体(在程序中可以假设为一直静态的物体):tv,refrigerator
            class_label == "prohibitory" || 
            class_label == "warning") {
            class_id = 1;
        }
