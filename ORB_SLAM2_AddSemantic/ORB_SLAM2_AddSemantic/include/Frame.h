...
public:
     ...
     *  
     */
    Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const long double &timeStamp, ORBextractor* extractorLeft, ORBextractor* extractorRight, ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth);
     ...
     */
    Frame(const cv::Mat &imGray, const long double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth);
    ...
    //反投影物体中心点
    void UnprojectObjectCenter(const int &box_id , const cv::Mat &imDepth );
...
public:
    ...
    //******* dynamic objects detect
    // vector<std::shared_ptr<Object>> objects_cur_detect_;//用于光流的物体
    cv::Mat imDepth;

    vector<std::shared_ptr<Object>> objects_cur_;//存储帧中的object 
    ...
    vector<bool> vbInDynamic_mvKeys;//位于动态物体区域内的特征点(未做动态检验):only this is useful

    //*******

    //*********draw object road sign
    // vector<double> test_frame; 
    vector<cv::Mat> vMcenter_static;
