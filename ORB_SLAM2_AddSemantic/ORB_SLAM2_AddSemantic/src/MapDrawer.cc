...
#include "MapPoint.h"
#include "KeyFrame.h"
#include <pangolin/pangolin.h> //Pangolin是对OpenGL进行封装的轻量级的OpenGL输入/输出和视频显示的库。
#include <mutex>
...
//************
void MapDrawer::DrawObjStatic(){
    //取出所有的地图点
    const vector<MapPoint*> &vpMObjCenters = mpMap->GetAllObjCenters() ;
    // cout << "vpMObjCenters size is : " << vpMObjCenters.size() << endl;
    if(vpMObjCenters.empty())
        return;
    
    const float &w = mKeyFrameSize;
    const float h = w*3.75;
    const float z = w*3.0;
    glBegin(GL_POINTS);
    glColor3f(1.0,0,1.0);         //yellow -RGB

    for(size_t i=0, iend=vpMObjCenters.size(); i<iend;i++)
    {
        // 不包括ReferenceMapPoints（局部地图点）
        if(vpMObjCenters[i]->isBad())
            continue;
        cv::Mat pos = vpMObjCenters[i]->GetWorldPos();
        // cv::Mat pos_inv = pos.t();
        float x = pos.at<float>(0);
        float y = pos.at<float>(1);
        float d = pos.at<float>(2);
        glBegin(GL_LINES);
        // glVertex3f(0,0,0);
        // glVertex3f(x,y,z);
        // glVertex3f(x,y-h,z+z2);
        // glVertex3f(x,y-h,z-z2);
        // glVertex3f(x,y+h,z+z2);
            glVertex3f(x,y,d);
            glVertex3f(x+w,y+h,d+z);
            glVertex3f(x,y,d);
            glVertex3f(x+w,y-h,d+z);
            glVertex3f(x,y,d);
            glVertex3f(x-w,y-h,d+z);
