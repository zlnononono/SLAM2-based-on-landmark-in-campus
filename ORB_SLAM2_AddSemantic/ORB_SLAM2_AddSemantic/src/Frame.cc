...
Frame::Frame(const Frame &frame)
    :mpORBvocabulary(frame.mpORBvocabulary), 
     ...
     objects_cur_(frame.objects_cur_)				//深拷贝
{
	//逐个复制，其实这里也是深拷贝
...
Frame::Frame(const cv::Mat &imGray, 
    ...
    :imDepth(imDepth), mpORBvocabulary(voc),mpORBextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
     mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth)
{
    // number will update here
    // double a =1.0;
    // test_frame.push_back(a);
    // cout << "test_frame.size()" << test_frame.size() << endl;
    ...
    for (int k=0; k<detect_result.size(); ++k){
        ...
        // vector<float> obj_center; 
        // cv::Mat obj_3d;
        // obj_3d = UnprojectObjectCenter(k,imDepth);
        // cout << "obj_3d " << obj_3d << endl;
    }
    //*****
    ...
    for (int k=0; k<mvKeys.size(); ++k){
        if (IsInDynamic(k) == true && IsInStatic(k) == false ){ //在动态物体框内,但不在静态物体框内
            ...
            // cout << "true" << endl;
            mvKeys[k] = cv::KeyPoint(-100,-100,-1);
        }
        else{
 ...
 */
Frame::Frame(const cv::Mat &imGray, const long double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth)
...
void Frame::ExtractORB(int flag, const cv::Mat &im)
{
    ...
    if(flag==0)
        ...
        (*mpORBextractorLeft)(im,				//待提取特征点的图像
							  cv::Mat(),		//掩摸图像, 实际没有用到//***InputArrary类型可以用很多种类型来初始化,包括Mat vector等.cv::Mat()传入空参数
...
cv::Mat Frame::UnprojectStereo(const int &i)
{
    ...
    if(z>0)
    {
		...
		//计算在当前相机坐标系下的坐标
        // cout << "here is true invfx : " << invfx << endl;
        const float x = (u-cx)*z*invfx;
        const float y = (v-cy)*z*invfy;
		//生成三维点（在当前相机坐标系下）
        cv::Mat x3Dc = (cv::Mat_<float>(3,1) << x, y, z);
		//然后计算这个点在世界坐标系下的坐标，这里是对的，但是公式还是要斟酌一下。首先变换成在没有旋转的相机坐标系下，最后考虑相机坐标系相对于世界坐标系的平移
        // cout << "here is true mRwc : " << mRwc << endl;
        // cout << "here is true mOw : " << mOw << endl;
...
//反投影物体中心点
void Frame::UnprojectObjectCenter(const int &box_id , const cv::Mat &imDepth){
    vector<float> obj_center;
    cv::Mat x3Dc = (cv::Mat_<float>(3,1) << -1, -1, -1);
    cv::Mat matCenter3d ;
    
    
        float u_center = (objects_cur_[box_id]->vdetect_parameter[0] + objects_cur_[box_id]->vdetect_parameter[2])/2.0;
        float v_center = (objects_cur_[box_id]->vdetect_parameter[1] + objects_cur_[box_id]->vdetect_parameter[3])/2.0;
        float z_center = imDepth.at<float>(v_center,u_center);

        if (z_center > 0 ){
            const float x = (u_center-cx)*z_center*invfx;
            const float y = (v_center-cy)*z_center*invfy;

	        //生成三维点（在当前相机坐标系下）
            x3Dc = (cv::Mat_<float>(3,1) << x, y, z_center);

            matCenter3d = mRwc*x3Dc+mOw;
            // cout << "matCenter3d:  " << endl << matCenter3d << endl;
            vMcenter_static.push_back(matCenter3d);
            // return matCenter3d;
        }
        // else
        //     return cv::Mat();

	    //计算在当前相机坐标系下的坐标 guiyihua multi z(where is my chinese input??)
