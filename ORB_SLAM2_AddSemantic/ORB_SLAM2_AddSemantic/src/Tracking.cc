    ...
    const double &timestamp)        //时间戳
{
    ...
    // 这里考虑得十分周全,甚至连四通道的图像都考虑到了,png格式是四通道的，多了一个透明度通道
 ...
 * Step 3 ：跟踪
 */
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im,const double &timestamp)//图像预处理
{
    mImGray = im;
...
void Tracking::Track()
{
    ...
    // Step 1：初始化
    if(mState==NOT_INITIALIZED)//**mState表示当前tracking的状态，有五个状态，见enum eTrackingState
    {
        if(mSensor==System::STEREO || mSensor==System::RGBD)
            ...
            //单目初始化
            MonocularInitialization();//***见下

        //更新帧绘制器中存储的最新状态
        mpFrameDrawer->Update(this); //this可以传递当前Tracking中所有的变量.(this在类中可以访问任何东西,使用this->即可)
    ...
    else//**只存在ＯＫ、ＬＯＳＴ两种可能了，ＯＫ则进行ＳＬＡＭ，ＬＯＳＴ则重定位
    {
        ...
        if(!mbOnlyTracking)//****表示正常SLAM模式（定位+地图更新）
        {
            ...
            if(mState==OK)
            {
                ...
                // 第一个条件,如果运动模型为空,说明是刚初始化开始，或者已经跟丢了;***见Step 5：跟踪成功，更新恒速运动模型
                // 第二个条件,如果当前帧紧紧地跟着在重定位的帧的后面，我们将重定位帧来恢复位姿
                // mnLastRelocFrameId 上一次重定位的那一帧
                if(mVelocity.empty() || mCurrentFrame.mnId<mnLastRelocFrameId+2)
                {
                    // ***TrackReferenceKeyFrame()作用：按照关键帧来进行Track，从关键帧中查找Bow相近的帧，进行匹配优化位姿
                    // 1）按照关键帧进行Track的方法和运动模式恢复相机运动位姿的方法接近。首先求解当前帧的BOW向量。2）再搜索当前帧和关键帧之间的关键点匹配关系，如果这个匹配关系小于15对的话，就Track失败了。
                    // 3）接着将当前帧的位置假定到上一帧的位置那里 4）并通过最小二乘法优化相机的位姿。5）最后依然是抛弃无用的杂点，当match数大于等于10的时候，返回true成功。
                    bOK = TrackReferenceKeyFrame();//**和上一个关键帧进行匹配
                }
                else
                {
                    // ***TrackWithMotionModel的作用：1）先通过上一帧的位姿和速度预测当前帧相机的位姿 
                    // 2）通过PnP方法估计相机位姿，在将上一帧的地图点投影到当前固定大小范围的帧平面上，如果匹配点少，那么扩大两倍的采点范围。
                    // 3）然后进行一次BA算法，通过最小二乘法优化相机的位姿
                    // 4）优化位姿之后，对当前帧的关键点和地图点，抛弃无用的杂点，剩下的点供下一次操作使用。若多点被观察数大于１０返回true,否则false
            ...
            else    
            {
                ...
                else//**mbVO为true表明此帧匹配了很少的MapPoints，少于10个，要跪的节奏
                {
                    ...
                    if(bOKMM && !bOKReloc)
                    {
                        ...
                        if(mbVO)//**当进行纯定位时才会有的一个变量,为false表示该帧匹配了很多的地图点,跟踪是正常的;如果少于10个则为true,表示快要完蛋了
                        {
                            // 更新当前帧的地图点被观测次数
        ...
        if(!mbOnlyTracking)
        {
            if(bOK)//作用：通过投影，从已经生成的地图点中找到更多的对应关系，精确结果
        ...
        if(bOK)
        {
            ...
            if(!mLastFrame.mTcw.empty())
            {
                ...
                cv::Mat LastTwc = cv::Mat::eye(4,4,CV_32F);//***Returns an identity matrix of the specified size and type.返回指定大小的单位矩阵
        ...
        //**********
        // cout << "hello " << endl;
        mpMap->EraseAllObjCenters();
        // cv::Mat center3d;
        int a = 0;
        for (int k=0; k<mCurrentFrame.objects_cur_.size(); ++k){
            // cv::Mat obj_3d;
            if (mCurrentFrame.objects_cur_[k]->ndetect_class == 1 ){
                mCurrentFrame.UnprojectObjectCenter(k,mCurrentFrame.imDepth);
            }
        }

        for (int a=0; a<mCurrentFrame.vMcenter_static.size(); ++a){
            // cout << "mCurrentFrame.vMcenter_static[a] is : " << endl << mCurrentFrame.vMcenter_static[a] << endl;
            MapPoint* pNewMP = new MapPoint(mCurrentFrame.vMcenter_static[a] ,mpMap,&mCurrentFrame,a);
            // vStaticObj_Center.push_back(pNewMP);
            mpMap->AddObjCenters(pNewMP);
            // cout << "!1" << endl;
        }
        ...
        //*************
            ...
            if(NeedNewKeyFrame())//***作用：判断是否需要生成新的关键帧，确定关键帧的标准
                CreateNewKeyFrame();
...
void Tracking::CreateNewKeyFrame()
{
    ...
    if(mSensor!=System::MONOCULAR)
    {
        ...
        //         // MapPoint* pNewMP = new MapPoint(mCurrentFrame.vMcenter_static[a],pKF,mpMap);
        ...
        // for (int a=0;a<mCurrentFrame.vMcenter_static.size();++a){
        //     cout << "a is " << a << endl;
        //     cout <<" mCurrentFrame.vMcenter_static[a] "<< endl << mCurrentFrame.vMcenter_static[a] << endl;
        // }
